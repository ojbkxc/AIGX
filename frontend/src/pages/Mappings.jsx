  ...
  return (
    <div>
      <div className="page-header">
        <h1>模型映射</h1>
        <p>配置网关的模型名称映射，将客户端请求的模型名映射到 Cloudflare Workers AI 模型</p>
      </div>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div className="card-header">
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <input className="form-input" style={{ width: 200, margin: 0 }} placeholder="搜索模型..."
              value={filter} onChange={(e) => setFilter(e.target.value)} />
            <button className="btn btn-outline btn-sm" onClick={() => setShowDefaults(!showDefaults)}>
              {showDefaults ? '收起' : '展开'} ({filteredDefaults.length})
            </button>
          </div>
        </div>
        {showDefaults && (
          <div className="card-body">
            <div className="default-models-grid">
              ...
              {filteredDefaults.length === 0 && (
                <div className="empty-state"><p>无匹配的默认模型</p></div>
              )}
            </div>
          </div>
        )}
        ...
        <div className="card-body">
          {entries.length === 0 ? (
            <div className="empty-state">
              <p>暂无自定义映射，默认模型映射已覆盖常见模型</p>
              <button className="btn btn-primary" onClick={addEntry}>添加自定义映射</button>
            </div>
          ) : (
