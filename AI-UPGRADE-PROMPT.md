...
### 6.1 「百年不变」项目重构的头脑风暴方向

围绕第〇章北极星目标（一百年后依然完美），以下维度是参照项目都没有、但对「要做 100 年」的 AIGX 最关键的重构杠杆，鼓励在每轮迭代中持续扩充并落到可验证里程碑：

1. **可替换性工程（依赖负债量化）**：把 axum / reqwest / sqlx / tiktoken 等每个第三方 crate 做成「可替换性档案」——替换成本评估、自有 trait 隔离层、它停维护时的逃生路径。目标：核心领域逻辑（调度、计费、桥接翻译）零外部类型泄漏，任何单一依赖的死亡都不致命。
2. **数据百年格式（自描述持久化）**：所有落盘数据（config、日志、用量、兑换码、订单）带 schema 版本 + 自校验 + 迁移路径，做成「任何未来版本都能无损读取」的自描述格式，旧数据文件永不废弃。
3. **考古层（可观测性历史）**：把每一次设计决策、运行时行为、配置演进记录为可审计的「考古层」——未来维护者能追溯「为什么这么写」。`docs/` + `CHANGELOG-AI.md` + 代码内 why 注释三件套形成闭环。
4. **配置向前兼容（永不 break）**：旧 `config.toml` 在任何新版本必须可直接加载；配置 schema 版本化 + 自动迁移，未识别字段不报错、保留并透传。
5. **API 契约版本化**：对外契约（`/v1/chat/completions`、`/v1/messages`）一经发布不破坏，变更走新增版本而非修改；管理 API 变更需显式列出迁移说明。
6. **依赖冻结与 SBOM**：`Cargo.lock` 入库、依赖供应审计、软件物料清单（SBOM）——100 年后有人接手时能精确还原构建环境。
7. **测试即规范**：661 个测试作为项目行为的可执行定义，任何行为变更先改测试；补行为契约集成测试，让「正确性」不依赖个人记忆。
8. **无人值守运维（自愈）**：在已有断路器 + AIMD + 亲和路由基础上，向「故障自愈时间线」「渠道自动调参」「成本异常自动熔断」演进，让网关能长期无人干预运行。
9. **Rust 生态自给策略**：某能力 Rust 生态暂缺成熟库时，优先自有实现（200 行内不引 crate）；预判 5-10 年后哪些 crate 会停维护，提前规划替换。
10. **性能护城河的百年叙事**：无 GC 网关是相对 new-api（Go）的核心优势，持续深化——hedged request 渠道竞速、纳秒级调度、毫秒级 failover 看板，用 benchmark 而非口号证明。
...
4. **质量红线**：每阶段收尾必须 `cargo clippy --workspace` 零警告、`cargo test --workspace` 全绿、前端 `npm run typecheck` 通过、`npm run build` 成功；不得引入新的 unsafe 与未审计依赖。**AI 可自行 commit / push**；AI 的验证手段是本地 `cargo build --release` 编译出 exe 并实际运行验证（详见第八章第 7 条）。
...
7. **并行协作纪律（所有者明确要求）**：你不是唯一在改这个仓库的人——项目所有者与其他同事正在与你并行写代码。因此：
   - 动手改任何文件前，先重新读取它的**当前**内容，禁止凭记忆或上一轮会话的缓存直接编辑——文件可能已被同事改过。
   - 改动保持内聚、爆炸半径最小：不做全文件重排、批量重命名、无差别格式化等会制造大面积合并冲突的动作；重构范围严格限定在本轮任务之内。
   - 遇到看不懂但能编译通过的代码，不要删除或「顺手清理」——那可能是同事正在进行的工作；确属废弃时记入路线图提案，由所有者裁决。
   - 发现自己的计划与仓库现状冲突（签名被改、模块被移等），以仓库当前状态为准调整方案，不按旧方案强行覆盖。
   - 禁止一切破坏性的 git 写操作（checkout / restore / reset / stash 等）——工作区里可能有同事未提交的成果。
8. 涉及支付、鉴权、配额的安全敏感改动，参照 AIGX 已有的安全基线（webhook 验签、OAuth state 校验、管理端 verify_admin、Stripe 金额走 epay.price 倍率），不得降低安全水位。
9. **百年工程红线（北极星目标的落地条款）**：
...
7. **Git 纪律**：
   - **允许 AI 自行 commit / push**——每轮迭代验证通过后即可提交并推送；只提交自己改的文件，绝不 commit 他人未提交的成果。
   ...
   - 每轮迭代结束时，把验证结论（启动是否成功、验证了哪些路径、发现并修复了什么）写进迭代日志，供后续 review。
...
继续迭代 `D:\GitHub\AIGX` 项目（AI 中转网关）。

先读 `AGENTS.md` 第 0 节项目速览了解硬性约束与架构，再开工。

工作方式：

1. 开工前 `git status` 检查工作区，避让我另一个会话正在编辑的文件，只提交你自己改的文件
2. Rust 编译验证只走 GitHub CI（本地无工具链）；前端可本地 `cd frontend && npm run build` 验证
3. 每轮改动：改码 → 本地 tsc/build 验证 → commit+push → GitHub API 查 CI runs 全绿才算完成
4. 涉及部署：下载 CI artifacts 的 linux-amd64 产物，SSH 装到 104.223.65.202:/opt/aigx/aigx 并重启服务（注意 config 重置坑）
5. 功能验证用 tests/e2e/run-e2e.mjs 跑浏览器 E2E（12 项断言），或 curl 直连生产 API

我的需求是：[在这里写本轮要迭代的具体内容]
