# AIGX Docker 镜像 - Linux AMD64 (生产环境)
# 优化的多用途镜像，平衡安全性和性能

# 构建阶段 - Rust 后端
FROM mcr.microsoft.com/mirror/docker/library/rust:1.75-bookworm AS builder-amd64
...
# 从构建阶段复制二进制文件
COPY --from=builder-amd64 /app/target/release/aigx /app/aigx
